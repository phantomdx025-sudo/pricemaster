...
def _parse_ledger_sheet(ws, party_type: str, log_callback=None) -> list[dict]:
    ...
    for raw_row in ws.iter_rows(min_row=4, values_only=True):
        ...
        # Defensive guard (mirrors BUG-1 fix in _parse_outstanding_sheet):
        # some Tally exports insert a "Total outstanding …" summary row in the ledger sheet
        # between parties. Skip it before _is_party_name_row so current_party isn't overwritten.
...
def _parse_outstanding_sheet(ws, party_type: str, log_callback=None) -> list[dict]:
    ...
    for raw_row in ws.iter_rows(min_row=2, values_only=True):
        ...
        # GUARD (BUG-1 fix): skip "Total outstanding …" summary text rows BEFORE
        # _is_party_name_row. These rows have text in col 0 and all other cols None —
        # the same shape as a real party-name row — so without this guard the parser
        # overwrites current_party with the summary text, corrupting all outstanding rows.
        if isinstance(row[0], str) and row[0].strip().startswith("Total outstanding"):
            continue
