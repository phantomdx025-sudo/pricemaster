...
class FinancialSyncTab(ctk.CTkFrame):
    ...
    def _confirm_sync_debtors(self):
        ...
        try:
            ...
            if result is None:
                self._log("⚠️  Could not read preview from file (it may still be valid)")
                if not messagebox.askyesno(
                    "Sync Debtors",
                    "Could not read preview from file (it may still be valid).\n\nProceed with sync?",
                ):
                    self._log("⏹ Debtors sync cancelled by user.")
                    return
            else:
                parties, txns = result
                self._log(f"✓ Preview: {parties} parties, ~{txns} transactions found")
                if not messagebox.askyesno(
                    "Sync Debtors",
            ...
            self._log(f"❌ Error reading Excel preview: {e}")
        ...
        try:
            ...
            if result is None:
                ...
                if not messagebox.askyesno(
                    ...
                    self._log("⏹ Creditors sync cancelled by user.")
                    return
            else:
                parties, txns = result
                self._log(f"✓ Preview: {parties} parties, ~{txns} transactions found")
        ...
        try:
            ...
            if count is None:
                ...
                if not messagebox.askyesno(
                    ...
                    self._log("⏹ Address Book sync cancelled by user.")
                    return
            else:
                self._log(f"✓ Preview: {count} address entries found")
                if not messagebox.askyesno(
                    "Sync Address Book",
    ...
    def _confirm_sync_all(self):
        ...
        # Build preview
        self._log("🔍 Scanning all files for preview…")
        try:
            ...
            self._log(f"✓ Debtors: {d_str}")
            self._log(f"✓ Creditors: {c_str}")
            self._log(f"✓ Address Book: {a_str}")
            ...
            if not messagebox.askyesno(
                ...
                self._log("⏹ Sync All cancelled by user.")
                return
        except Exception as e:
            self._log(f"❌ Error reading Excel previews: {e}")
            messagebox.showerror("Error", f"Could not read Excel files: {e}")
            return
    ...
    def _run_sync(self, file_type: str, path: str):
        ...
        def worker():
            ...
            msg = ""
            try:
                self._log(f"⏳ Processing {file_type}…")
        ...
        def worker():
            ...
            try:
                ...
                try:
                    ...
                    import traceback
                    self._log(f"❌ Debtors sync failed: {e}")
                    self._log(f"Stack trace: {traceback.format_exc()}")
                    results.append(("Debtors", False, str(e)))

                self._log("\n[2/3] Syncing Creditors…")
                self._log(f"📁 File: {creditors_path}")
                try:
                    ...
                    import traceback
                    self._log(f"❌ Creditors sync failed: {e}")
                    self._log(f"Stack trace: {traceback.format_exc()}")
                    results.append(("Creditors", False, str(e)))

                self._log("\n[3/3] Syncing Address Book…")
                self._log(f"📁 File: {address_path}")
                try:
                    ...
                    import traceback
                    self._log(f"❌ Address Book sync failed: {e}")
                    self._log(f"Stack trace: {traceback.format_exc()}")
                    results.append(("Address Book", False, str(e)))
