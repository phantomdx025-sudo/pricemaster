...
def _ensure_local_schema(conn, log_callback=None):
    """Ensure local database has all required columns (migrate schema if needed)."""
    cur = conn.cursor()
    
    # ── Check and add position column to inv_categories ────
    try:
        cur.execute("SELECT position FROM inv_categories LIMIT 1")
    except sqlite3.OperationalError:
        _log(log_callback, "📝 Adding position column to inv_categories…")
        cur.execute("ALTER TABLE inv_categories ADD COLUMN position INTEGER DEFAULT 0")
        conn.commit()
    
    # ── Check and add position column to inv_tabs ────
    try:
        cur.execute("SELECT position FROM inv_tabs LIMIT 1")
    except sqlite3.OperationalError:
        _log(log_callback, "📝 Adding position column to inv_tabs…")
        cur.execute("ALTER TABLE inv_tabs ADD COLUMN position INTEGER DEFAULT 0")
        conn.commit()
...
def push(db_path: str, supabase_url: str, service_role_key: str, log_callback=None) -> tuple[bool, str]:
    ...
    try:
        ...
        _ensure_local_schema(conn, log_callback)
