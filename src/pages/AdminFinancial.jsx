...
export default function AdminFinancialContent() {
  ...
  const {
    ...
    fetchAllPartyLabels, fetchPartyLabel, setPartyLabel, fetchCustomLabels, addCustomLabel, deleteCustomLabel,
  } = fin
  ...
  // Called by both FinPartyList (label pill) and FinSlideOver (header label picker)
  // Keeps debtorLabelMap/creditorLabelMap in sync without a refetch
  ...
  // partyLabelKey is resolved at render time for the currently-open party
  const currentLabelMap = selectedParty?.party_type === 'creditor' ? creditorLabelMap : debtorLabelMap
  const currentPartyLabelKey = selectedParty ? (currentLabelMap.get(selectedParty.party_name) ?? null) : null
  ...
  const slideOverHooks = {
    ...
    partyLabelKey: currentPartyLabelKey,
  }
